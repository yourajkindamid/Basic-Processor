...
vector <string> totalops = {"STA","INP","LDA","ADD","AND","OR_fn","SUB","SWP","CMP","LDB","STB","NEG","SEA","JMP",
                             "BSA","STZ","NOT","CLA","ROTL","ROTR","ROTLC","ROTRC","BPA","DMG","ENCR","DECR",
                             "LOADB","STOREB","ADDB","SUBB","ANDB","ORB","MOVB","OUT","ION","IOF", "MULTB", "DIVB"};
...
void ENCR(int x)
{
    ...
    if(key != x)
    {
        text = (x - key)/10;        // can work with a key shift of upto 9
        ACCM[0] = (text + key) % 26;
        OPR[0] = (text + key) % 26;
    }
    else if(key == x)
...
{
    IPR[0] = x;
    int key = x % 10;           //works on basically the same constraints for decryption aswell
    int text = 0;
    if(key != x)
    {
        text = (x - key)/10;        
...
void BSA(int x)
{
    DATARAM[x]=exec_counter+1; 
    exec_counter=x; 
}
void BPA(int x)
{if(ACCM[0]==0) exec_counter=x-1;}
...
void howmanyins(){
    cout<<"\t\t\tWelcome to the 4097 x 19 Processor"<<endl<<endl;
